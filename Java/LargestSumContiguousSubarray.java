...
public class LargestSumContiguousSubarray {
	...
	// method1: brute force
	public int maxSubarraySum000(int[] nums) {
	...
	// greedy based on accumulated sum, find the max diff in accSum Array
	public int max(int[] nums) {
		...
		for (int i = 0; i < nums.length; i++) {
			accSum += nums[i];
			maxSum = Math.max(maxSum, accSum - min);
			min = Math.min(min, accSum);
		}
